...
if (isset($_REQUEST['course_url'])) {
    ...
    do {
        foreach ($events as $event) {
            $api->delete(
                ...
                array(
                    'cancel_reason' => $toolbox->config('TOOL_NAME') . " course_url={$_REQUEST['course_url']}"
                )
            );
        }
    ...
    $smarty->assign(
        ...
        '<form action="' . $_SERVER['PHP_SELF'] . '" method="post">' .
            'Course URL <input id="course_url" name="course_url" type="text" />' .
            '<input type="submit" value="Purge  All Calendar Events" />' .
        '</form>'
    );
}
